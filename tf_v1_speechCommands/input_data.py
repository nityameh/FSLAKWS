...
class AudioProcessor(object):
  ...
  def prepare_processing_graph(self, model_settings, summaries_dir):
    ...
    with tf.compat.v1.get_default_graph().name_scope('data'):
      ...
      foreground_rms = tf.sqrt(tf.reduce_mean(tf.square(
          sliced_foreground)))
      background_rms = tf.sqrt(tf.reduce_mean(tf.square(
          self.background_data_placeholder_)))
      ...
      background_data_scaled = tf.multiply(
          self.background_data_placeholder_, snr_scaling)

      self.background_volume_placeholder_ = tf.compat.v1.placeholder(
          tf.float32, [], name='background_volume')
      background_mul = tf.multiply(background_data_scaled,
